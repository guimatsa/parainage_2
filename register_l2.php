<?php
    if(isset($_POST['reg'])){
        ...
        $query=$db->query("INSERT INTO l1 VALUES(NULL, '$nom', '$password', '$email', '$num_parainage', 'null', '$niveau')");
            $select = $db->query("SELECT * FROM l2 WHERE id=$num_parainage");
            $ligne= mysqli_num_rows($select);
            $parain=$db->mysqli_fetch_assoc($select);
            while($ligne=1){
                $nom_p=$ligne['nom_p'];
            }
            $updatel2=$db->query("UPDATE l2 SET fullieule=$nom where id=$num_parainage");
            $upadatel1=$db->query("UPDATE l1 set nom_parain=");
